(function ($) {
  $.fn.pafap_visualLightbox = function (options)
  {
    var activeImage = null, badObjects = ["select", "object", "embed"],
    groupName = null, imageArray = [], slideShowTimer = null, startImage = null,
    descriptionHeight = 50, imgPreloader, showTimer;
    if (!document.getElementsByTagName) {return;}
    options = $.extend(
    {
      animate:true, autoPlay:true, borderSize:39,
      containerID:document, enableSlideshow:true, googleAnalytics:false,
      imageDataLocation:"south", closeLocation:"", initImage:"", loop:true,
      overlayDuration:0.2, overlayOpacity:0.7, prefix:"", classNames:"vlightbox",
      resizeSpeed:7, showGroupName:false, slideTime:4, strings:
      {
        closeLink:"", loadingMsg:"loading", nextLink:"", prevLink:"", postComment:"Post Comment",
        startSlideshow:"", stopSlideshow:"", numDisplayPrefix:"", numDisplaySeparator:"/"
      },
    ...
    var playSlides = options.autoPlay;var container = $(options.containerID);
    ...
    objCommentContainer.setAttribute("id", getID("imageCommentContainer"));
    objImageDataContainer.appendChild(objCommentContainer);
    //textarea;
    var objCommentTextarea = document.createElement("textarea");
    objCommentTextarea.setAttribute("id", getID("imageCommentTextarea"));
    objCommentContainer.appendChild(objCommentTextarea);
    //comment button span;
    var objCommentSpan = document.createElement("span");
    objCommentSpan.setAttribute("id", getID("imagePostComment"));
    objCommentContainer.appendChild(objCommentSpan);
    //post comment button;
    var objPostCommentButton = document.createElement("a");
    objPostCommentButton.setAttribute("id", getID("imagePostComment"));
    objPostCommentButton.setAttribute("href", "javascript:void(0);");
    objPostCommentButton.innerHTML = options.strings.postComment;
    objCommentSpan.appendChild(objPostCommentButton);
    //current comment result;
    var objCurrentCommentResults = document.createElement("div");
    objCurrentCommentResults.setAttribute("id", getID("imageCurrentCommentResults"));
    objCommentContainer.appendChild(objCurrentCommentResults);
    //comment results;
    var objCommentResults = document.createElement("div");
    objCommentResults.setAttribute("id", getID("imageCommentResults"));
    objCommentContainer.appendChild(objCommentResults);
    /*comment container*/
    ...
    var start = this.start = function (imageLink) {
      ...
      if (options.startZoom) {$$("imageContainer").css({width:imageLink.width() + "px", height:imageLink.height() + "px"});
      ...
      $$("lightbox").css({
        ...
        width:imageLink.width() + options.borderSize * 2 + "px", height:"auto"});
      }
      else {
      ...
      $$("lightbox").show();
      imageArray = [];
      groupName = null;startImage = 0;
      $("." + imageLink.attr("className"), container).each(function () {
        if (getHref(this)) {
          imageArray.push({link:getHref(this), title:getTitle(this), rel:getRel(this)});
          if (this == imageLink.get(0)) {startImage = imageArray.length - 1;}
        }
      });
      if (imageArray.length > 1) {
        groupName = imageLink.attr("className");
      }
      if (options.featBrowser) {$(window).resize(adjustImageSizeNoEffect);}
      if (options.floating) {$(window).scroll(adjustImageSizeNoEffect);}
      $(window).resize(adjustOverlay);
      $(window).scroll(adjustOverlay);changeImage(startImage);
    };
    ...
    function changeImage(imageNum) {
      ...
      if (options.googleAnalytics) {urchinTracker(imageArray[activeImage].link);}
    }
    ...
    function adjustImageSize(recall, noImage) {
      ...
      if (recall == true) {
        ...
        if (options.floating) {moveEffect($$("lightbox"), imgTop);} else {$$("lightbox").css({top:imgTop + "px"});}
      }
      else {
        ...
        var lightboxImage2;
        if (options.startZoom && lightboxImage.attr("src")) {
          lightboxImage2 = lightboxImage;lightboxImage2.attr({id:getID("lightboxImage2")});
        }
        else {
          lightboxImage.remove();
        }
        if (!noImage) {
          lightboxImage = $(imgPreloader);lightboxImage.hide();lightboxImage.attr({id:getID("lightboxImage")});
        ...
        if (!noImage) {
          if (options.startZoom) {
            ...
            if (lightboxImage2) {
              with (lightboxImage2) {css({width:1 > resizeFactor ? "auto" : width() / parent().width() * 100 + "%", height:1 > resizeFactor ? height() / parent().height() * 100 + "%" : "auto", left:0, top:0});}}lightboxImage.css({opacity:0, display:"block", position:"absolute", width:1 > resizeFactor ? zoomF + "%" : "auto", height:1 > resizeFactor ? "auto" : zoomF + "%", left:(100 - zoomF * (1 > resizeFactor ? 1 : resizeFactor)) / 2 + "%", top:(100 - zoomF * (1 > resizeFactor ? 1 / resizeFactor : 1)) / 2 + "%"});
          }
    ...
    function showLoading() {
      ...
      showTimer = setTimeout(function () {$$("loading").css({visibility:"visible"});}, 300);
    }
    function hideLoading() {clearTimeout(showTimer);$$("loading").hide();}
    function showImage() {
      hideLoading();
      if (options.startZoom) {$$("overlay:hidden").css({opacity:0}).show().fadeTo(overlayDuration * 1000, options.overlayOpacity);updateDetails();}
      else {$$("lightboxImage").css({opacity:0}).show().fadeTo(500, 1, function () {updateDetails();});}preloadNeighborImages();
    }
    function updateDetails() {
      ...
      if (imageArray.length > 1) {
        ...
        }$$("numberDisplay").text(num_display).show();if (!enableSlideshow) {$$("slideShowControl").hide();}$$("detailsNav").show();
      }
      $$("imageDataContainer").animate({height:"show", opacity:"show"}, 650, null, function () {updateNav();});
    }
    function updateNav() {
      var d = 1 / imageArray.length;descriptionHeight = descriptionHeight * (1 - d) + $$("imageDataContainer").height() * d;
      if (imageArray.length > 1) {
        $$("hoverNav").show();
        if (enableSlideshow) {if (playSlides) {startSlideShow();} else {stopSlideShow();}}
      }
      enableKeyboardNav();
    }
    function startSlideShow() {
      if ($$("lightbox:hidden").length) {return;}
      pauseSlideShow();playSlides = true;
    ...
    function toggleSlideShow() {if (playSlides) {stopSlideShow();} else {startSlideShow();}}
    function pauseSlideShow() {if (slideShowTimer) {slideShowTimer = clearTimeout(slideShowTimer);}}
    //show next;
    function showNext() {
      $$('imgselected').val(imageArray[activeImage]);
      if (imageArray.length > 1) {pauseSlideShow();
      if (!options.loop && (activeImage == imageArray.length - 1 && startImage == 0 || activeImage + 1 == startImage)) {end();return;}
      if (activeImage == imageArray.length - 1) {changeImageWithData(0);$$('imgselected').val('');$$('imgselected').val(imageArray[0].link);} else {changeImageWithData(activeImage + 1);$$('imgselected').val('');$$('imgselected').val(imageArray[activeImage + 1].link);}}
    }
    function changeImageWithData(imageNum) {
      $$("imageDataContainer").animate({height:"hide", opacity:"hide"}, 650, null, function () {changeImage(imageNum);});
    }
    //show prev;
    function showPrev() {
      if (imageArray.length > 1) {
        if (activeImage == 0) {changeImageWithData(imageArray.length - 1);$$('imgselected').val('');$$('imgselected').val(imageArray[imageArray.length - 1].link);}
        else {changeImageWithData(activeImage - 1);$$('imgselected').val('');$$('imgselected').val(imageArray[activeImage - 1].link);}
      }
    }
    function showFirst() {if (imageArray.length > 1) {changeImageWithData(0);}}
    function showLast() {if (imageArray.length > 1) {changeImageWithData(imageArray.length - 1);}}
    ...
    function keyboardAction(e) {
      ...
      if (key == "x" || key == "o" || key == "c") {end();}
      else if (key == "p" || key == "%") {showPrev();}
      else if (key == "n" || key == "'") {showNext();}
      else if (key == "f") {showFirst();}
      else if (key == "l") {showLast();}
      else if (key == "s") {if (imageArray.length > 0 && options.enableSlideshow) {toggleSlideShow();}}*/
    }
    ...
    function end(Event) {
      ...
      disableKeyboardNav();
      pauseSlideShow();$$("lightbox").hide();
      if (options.overlayOpacity) {$$("overlay").fadeOut(overlayDuration * 1000);}
      else {$$("overlay").hide();}
      $(window).unbind("resize", adjustImageSizeNoEffect);
      $(window).unbind("scroll", adjustImageSizeNoEffect);
      $(window).unbind("resize", adjustOverlay);$(window).unbind("scroll", adjustOverlay);showBadObjects();
    }
    function adjustImageSizeNoEffect() {adjustImageSize(true);}
    function adjustOverlay() {$$("overlay").css({left:getPageScroll().x + "px", top:0, width:"100%", height:docWH()[1] + "px"});}
    function showBadObjects() {
      var els;var tags = badObjects;
      for (var i = 0; i < tags.length; i++) {
        els = document.getElementsByTagName(tags[i]);
        for (var j = 0; j < els.length; j++) {$(els[j]).css({visibility:"visible"});}
      }
    }
    function hideBadObjects() {var tags = badObjects;for (var i = 0; i < tags.length; i++) {$(tags[i]).css({visibility:"hidden"});}}
    function getPageScroll() {
      ...
      } else if (document.body) {x = document.body.scrollLeft;y = document.body.scrollTop;}return {x:x, y:y};
    }
    function getPageSize() {
      ...
      if (self.innerHeight) {
        ...
        windowX = document.documentElement.clientWidth;windowY = document.documentElement.clientHeight;
      }
      else if (document.body) {
        windowX = document.body.clientWidth;windowY = document.body.clientHeight;
      }
    ...
    function docWH() {
      ...
      if (e) {
        w = Math.max(w, e.scrollWidth, e.offsetWidth);h = Math.max(h, e.scrollHeight, e.offsetHeight);
      }
      if (b) {
...
var Lightbox = (new jQuery).pafap_visualLightbox($VisualLightBoxParams$);
var tmpNext;