 ...
 */
jQuery.pafapListSelection = function(elem, width, height, row){
    ...
    var bindEventsOnItems = function(elem){
        $.each(elem.children("li").children(".pafaplist_item"), function(i, obj){
            obj = $(obj);
    ...
    var wrapElements = function(elem, width, height, row){
        elem.children("li").wrapInner('<div class="pafaplist_item"></div>');
        $(".pafaplist_item").css("height", height + "px");
        var newwidth = Math.ceil((parseInt(width, 10)) / parseInt(row, 10)) - 11;
        $(".pafaplist_item").css("width", newwidth + "px");
    }
