	...
	function upload_multi_files () {
		...
		if ($this->count_files()) {
			foreach ($this->names_array as $key => $value) {
				if ($value != "") {
					$this->the_file = $value;
					$new_name = $this->set_file_name('', $_SESSION['record']);
...
if(isset($_SESSION['record']))
{
  ...
  if(isset($_FILES['pupload']))
  {
    $multi_mysql = new pafap_mysql();
    $single_upload->tmp_names_array = $_FILES['pupload']['tmp_name'];
    $single_upload->names_array = $_FILES['pupload']['name'];
    $single_upload->error_array = $_FILES['pupload']['error'];
    ...
    foreach($single_upload->renamed as $key=>$ren)
    {
      if ($single_upload->process_image($ren)){
        $sql = "UPDATE pafap_profile SET image = 'users/ImagesPool/{$single_upload->sd($ren)}' WHERE uid = '{$recuid}'";
